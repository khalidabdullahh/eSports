...
/**
 * Service to fetch and manage tournaments authoritatively from Supabase PostgreSQL.
 * Strict rule: NEVER silently fall back to mock/demo data in production.
 */
export class TournamentService {
  ...
  static async getTournaments(includeDrafts = false): Promise<Tournament[]> {
    // Production path or configured environment: query real database
    if (isProduction || isSupabaseConfigured) {
      try {
        ...
        if (error) {
          console.error("Database query error in TournamentService.getTournaments:", error.message);
          // In production, return empty list on error to prevent showing fake mock data
          return [];
        }

        if (!data || data.length === 0) {
          // Authentic empty state: return empty array, do NOT show fake demo cups
          return [];
        }
        ...
        console.error("Unexpected error in TournamentService.getTournaments:", err);
        return [];
      }
    }

    // Isolated developer prototype fallback (STRICTLY non-production offline mode only)
  ...
  static async getTournamentById(idOrSlug: string): Promise<Tournament | null> {
    if (isProduction || isSupabaseConfigured) {
      try {
        ...
        console.error("Unexpected error in TournamentService.getTournamentById:", err);
        return null;
      }
    }
